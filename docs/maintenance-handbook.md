...
장애 기록에는 비밀번호, 토큰, 이메일 원문, 게시글·상담·일지 원문을 붙이지 않는다. 사용자 식별이 필요하면 내부 사용자 번호도 최소한으로 사용하고 외부 이슈에는 기록하지 않는다.
...
1. 코드는 브랜치와 PR로 바꾸고, 서버 파일을 직접 고치지 않는다.
2. secret은 AWS Secrets Manager에서 바꾸고 재배포하며, 값은 Git·문서·채팅에 남기지 않는다.
3. DB 구조 변경 전에는 snapshot과 복원 방법을 먼저 확인한다. 이미지 rollback만으로 migration은 되돌아가지 않는다.

작업 증거는 `배포 SHA / 실행 시각 / migration 유무 / Actions 주소 / smoke 결과 / rollback 여부`만 기록한다. 이 정보면 면접이나 인수인계에서도 변경 근거와 장애 대응 과정을 설명할 수 있다.
