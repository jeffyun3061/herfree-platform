...
| POST | `/api/posts/images/upload-url` | 게시글 이미지 presigned 업로드 URL 발급 | 회원 |
...
### 게시글 이미지 업로드 (`POST /api/posts/images/upload-url`)

클라이언트가 S3에 직접 PUT 업로드하기 위한 presigned URL을 발급한다. 익명 글 작성 시에도 이미지 첨부 가능.
...
- `contentType`: `image/jpeg`, `image/png`, `image/webp`만 허용
- `contentLength`: 1 ~ 10,485,760 (10MB)
...
{
  "uploadUrl": "https://...",
  "imageUrl": "https://{bucket}.s3.{region}.amazonaws.com/posts/{userId}/{uuid}.jpg",
  "objectKey": "posts/{userId}/{uuid}.jpg"
}
```

업로드 순서: (1) presigned URL 발급 → (2) `uploadUrl`로 PUT → (3) 게시글 작성/수정 시 `imageUrl` 전달
