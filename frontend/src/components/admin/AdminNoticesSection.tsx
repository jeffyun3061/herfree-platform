...
export function AdminNoticesSection() {
  const { noticePage, isLoading, error, refetch } = useAdminNotices(50);
  ...
  return (
    <div className="space-y-5">
      <AdminPublishHeader
        ...
        note="목록 상단에 고정되어 보입니다. 숨기기는 임시 비노출, 삭제는 복구할 수 없습니다."
      />
      ...
      <div>
        <h3 className="mb-3 text-[13px] font-semibold text-cream-foreground">등록된 공지</h3>
        {isLoading && <LoadingSpinner />}
        {error && <ErrorMessage message={getErrorMessage(error)} />}
        <div className="space-y-2">
          {noticePage.content.map((item) => {
